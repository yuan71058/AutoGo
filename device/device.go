...
func heartbeat() {
	...
	// 主循环，每秒发送命令
	for {
		str2 := utils.Shell("dumpsys window displays | grep init=")
		if str2 != str1 {
			start := strings.LastIndex(str2, "cur=")
			if start > -1 {
				// 提取 cur= 后的分辨率字符串
				start += len("cur=")
				end := strings.Index(str2[start:], " ")
				if end == -1 {
					end = len(str2) // 如果没有空格，则到字符串末尾
				} else {
					end += start
				}
				resolution := str2[start:end]
				// 按 'x' 分割分辨率
...
func SetScreenMode(mode int) {
	java.CallJavaMethod("utils", "setScreenMode|"+i2s(mode))
}
