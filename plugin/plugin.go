...
func LoadApk(apkPath string) *Plugin {
	...
	str := java.CallJavaMethod("plugin", "loadApk|"+apkPath)
	if str == "" {
		return nil
	}
...
func (p *Plugin) NewInstance(className string, values ...interface{}) *Class {
	...
	str := java.CallJavaMethod("plugin", "newInstance|"+p.obj+"|"+className+"|"+list)
	if str == "" {
		return nil
	}
...
func (c *Class) Call(methodName string, values ...interface{}) *Obj {
	...
	return &Obj{obj: java.CallJavaMethod("plugin", "call|"+c.obj+"|"+methodName+"|"+list)}
}
